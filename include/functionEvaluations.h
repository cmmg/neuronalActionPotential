...
//
template <class T, int dim>
void evaluateVectorFunctionOnSurface(FEFaceValues<dim>& fe_face_values, unsigned int DOF, Table<1, T>& ULocal, Table<2, T>& U){
  unsigned int dofs_per_cell= fe_face_values.dofs_per_cell;
  ...
  for (unsigned int q=0; q<n_q_points; ++q){
    ...
    for (unsigned int k=0; k<dofs_per_cell; ++k){
      unsigned int ck = fe_face_values.get_fe().system_to_component_index(k).first - DOF;
      if (ck>=0 && ck<dim){
	U[q][ck]+=ULocal[k]*fe_face_values.shape_value(k, q); //U
      }
    }
  }
}
