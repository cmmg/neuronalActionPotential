...
//Basic framework for modeling spatio-temporal action potential propagation in neurons using electro-diffusive PNP model with myelin and peri-axonal space.
//Created May 2021
...
namespace elasticity1
{
  ...
  public:
	...
	if( (p[1]>1.6 || p[1] < 0.5) && p[0]>(start) && p[0]< (start+dimB) ){
	  values(0) = initialExtraConcNa;
	  values(1) = initialExtraConcK;
	  values(2) = initialExtraConcA;
	  values(3) = 0.00;
  ...
  template <int dim>
  void elasticity<dim>::refine_grid (int mode){
	...
	if (cell->is_locally_owned()){
	  ...
	  if (mode==2){
	    ...
	    if ( (cell->face(2)->center()[1]< (1.6+dimH) && cell->face(3)->center()[1]>1.6) || (cell->face(2)->center()[1]<0.5 && cell->face(3)->center()[1]>(0.5-dimH)) ){
	      mark_refine=true; //set refine                                                                                                                                                                       
	    }
	  }
